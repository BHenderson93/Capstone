    ...
    return(
        ...
        { moods.length === 0? 
            ...
            <form action="" onSubmit={handleLocationSubmit} className="container-medium">
                ...
                <input type="search" placeholder="Where are you? For example, 'seattle, wa' or 'new york'." className="text-center" value={welcomePageState.search} onChange={(e) => setWelcomePageState({ ...welcomePageState, search: e.target.value })} onFocus={(e) => e.target.placeholder = ""} onBlur={(e) => e.target.placeholder = "Where are you? For example, 'seattle, wa' or 'new york'."} />
                <br />
